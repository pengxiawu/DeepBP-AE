...
class BPAE(object):
    def __init__(self, sess, input_dim, emb_dim, decoder_num_steps, decoder_type):
        ...
        self.indices_x = tf.compat.v1.placeholder(tf.int64, [None, 2])
        self.values_x = tf.compat.v1.placeholder(tf.float32, [None])
        self.dense_shape_x = tf.compat.v1.placeholder(tf.int64, [2])
        self.input_x = tf.SparseTensor(indices=self.indices_x,
                                       values=self.values_x,
                                       dense_shape=self.dense_shape_x)
        self.encoder_weight = tf.Variable(tf.compat.v1.truncated_normal(
        ...
        ## encode the input
        self.encode_shape_placeholder = tf.compat.v1.placeholder("int64", [2], name="encode_shape")
        ## encode the input
        self.encode = tf.compat.v1.sparse_tensor_dense_matmul(self.input_x, self.encoder_weight)
        ## Add a noise layer
        # self.noise_sd = tf.Variable(0.01)
        # self.encode += self.noise_sd *tf.random_normal(self.encode_shape_placeholder)
        ...
        def decode_subgrad_bpsaec(x, W, num_steps, step_size):
            ...
            for i in range(num_steps):
                ...
                x = tf.compat.v1.layers.batch_normalization(x, axis=1)
            x = tf.nn.relu(x)
        ...
        self.sq_loss = tf.reduce_mean(tf.pow(tf.compat.v1.sparse_add(self.input_x,
                                      -self.pred), 2))*self.input_dim
        self.learning_rate = tf.compat.v1.placeholder("float", [])
        self.sq_optim = tf.compat.v1.train.GradientDescentOptimizer(
