...
flags.DEFINE_string("decoder_type", "SAEC", "choose one from [GAE, GAEC, SAE, SAEC]")
...
flags.DEFINE_string("checkpoint_dir", "./results/20200519_deepMIMOdataset_saec/",
...
for dataset_i in range(num_random_dataset):
    ...
    for experiment_i in range(num_experiment):
        ...
        saec_lp_err, saec_lp_exact, saec_lp_solve = \
            LP_BP_avg_err(np.transpose(learned_matrix), Y, X_test, use_pos=False)
        saec_lp_err_pos, saec_lp_exact_pos, saec_lp_pos_solve = \
            LP_BP_avg_err(np.transpose(learned_matrix), Y, X_test, use_pos=True)


        res = {}
        res['saec_lp_err'] = saec_lp_err
        res['saec_lp_exact'] = saec_lp_exact
        res['saec_lp_solve'] = saec_lp_solve
        res['saec_lp_err_pos'] = saec_lp_err_pos
        res['saec_lp_exact_pos'] = saec_lp_exact_pos
        res['saec_lp_solve'] = saec_lp_pos_solve
