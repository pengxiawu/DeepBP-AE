...
def LP_BP(X, input_dim, emb_dim):
    ...
    f_err, f_exact, f_solved = LP_BP_avg_err(np.transpose(F), Y, X, use_pos=False)
    f_err_pos, f_exact_pos, f_solved_pos = LP_BP_avg_err(np.transpose(F), Y, X, use_pos=True)
    ...
    res['lp_f_solved'] = f_solved
    res['lp_f_solved_pos'] = f_solved_pos
    # res['lp_p_err'] = p_err
